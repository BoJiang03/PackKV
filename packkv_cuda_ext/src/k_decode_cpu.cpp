...
void k_bits_warp_uncompact(
    const uint8_t compact_array[COMPACT_ARRAY_SIZE],
    uint8_t bits_array[HIDDEN_DIM_BLOCK_SIZE]
) {
    ...
    if constexpr (RAW_BIT_LEN == 3) {
        for (size_t pack_idx = 0; pack_idx < HIDDEN_DIM_BLOCK_SIZE; pack_idx += PACK_PER_THREAD * 2) {
            ...
            for (size_t i = 0; i < 3; i++) {
                two_warp_thread_bits[i] = compact_array[pack_idx / (PACK_PER_THREAD * 2) * 3 + i];
            }
            ...
            for (size_t i = 0; i < PACK_PER_THREAD; i++) {
                bits_array[pack_idx + i] = (warp_thread1 >> (i * 3)) & 0x07;
            }
            for (size_t i = 0; i < PACK_PER_THREAD; i++) {
                bits_array[pack_idx + PACK_PER_THREAD + i] = (warp_thread2 >> (i * 3)) & 0x07;
            }
        }
    } else {
        for (size_t pack_idx = 0; pack_idx < HIDDEN_DIM_BLOCK_SIZE; pack_idx += PACK_PER_THREAD) {
            ...
            uint32_t warp_thread = 0;
            for (size_t i = 0; i < COMPACT_BYTE_NUM; i++) {
                warp_thread |= (uint32_t)compact_array[pack_idx / PACK_PER_THREAD * COMPACT_BYTE_NUM + i] << (i * 8);
            }
            for (size_t i = 0; i < PACK_PER_THREAD; i++) {
                bits_array[pack_idx + i] = (warp_thread >> (i * RAW_BIT_LEN)) & ((1 << RAW_BIT_LEN) - 1);
            }
        }
    }
}
...
size_t k_encoded_packs_uncompact(
    uint64_t encoded_packs[HIDDEN_DIM_BLOCK_SIZE],
    ...
    for (size_t pack_idx = 0; pack_idx < HIDDEN_DIM_BLOCK_SIZE; pack_idx++) {
        ...
        // if (round_idx == 0 && pack_idx == 31) {
        //     std::cout << "round_idx: " << (int)round_idx << ", compact_array: ";
        //     for (size_t i = 0; i < offset + pack_byte_num; i++) {
        //         std::cout << (int)compact_array[i] << " ";
        //     }   
        //     std::cout << std::endl;
        // }
        // if (round_idx == 0 && pack_idx == 63) {
        //     std::cout << "round_idx: " << (int)round_idx << ", compact_array: ";
        //     for (size_t i = 0; i < offset + pack_byte_num; i++) {
        //         std::cout << (int)compact_array[i] << " ";
        //     }   
        ...
        encoded_packs[pack_idx_mapped] = encoded_pack;
...
void k_data_uncompact(
    ...
    copy_data((const uint8_t *)encode_data, (uint8_t *)round_size, ROUND_SIZE_BYTE_NUM);

    for (size_t round_idx = ROUND_NUM - 1; round_idx > 0; round_idx--) {
    ...
    for (size_t round_idx = 0; round_idx < ROUND_NUM; round_idx++) {
        k_bits_warp_uncompact<RAW_BIT_LEN, HIDDEN_DIM_BLOCK_SIZE, MIN_BYTE_NUM_PER_ROUND>(encode_data + offset, mins[round_idx]);

        // std::cout << "round_idx: " << (int)round_idx << std::endl;
        // for (size_t j = 0; j < HIDDEN_DIM_BLOCK_SIZE; j++) {
        //     std::cout << (int)mins[round_idx][j] << ", ";
        // }
        // std::cout << std::endl;

        offset += MIN_BYTE_NUM_PER_ROUND;
        k_bits_warp_uncompact<min_bits_for_encode_len(RAW_BIT_LEN), HIDDEN_DIM_BLOCK_SIZE, ENCODE_LEN_BYTE_NUM_PER_ROUND>(encode_data + offset, encode_lens[round_idx]);
        offset += ENCODE_LEN_BYTE_NUM_PER_ROUND;
        k_encoded_packs_uncompact<HIDDEN_DIM_BLOCK_SIZE>(encoded_packs[round_idx], encode_lens[round_idx], encode_data + offset, round_idx);
        offset += round_size[round_idx] - MIN_BYTE_NUM_PER_ROUND - ENCODE_LEN_BYTE_NUM_PER_ROUND;
    }
...
void k_block_decode_cpu(
    ...
    k_data_uncompact<RAW_BIT_LEN, ROUND_NUM, CTX_LEN_BLOCK_SIZE, HIDDEN_DIM_BLOCK_SIZE>(mins, encode_lens, encoded_packs, compressed_buffer);
...
void k_decode_cpu(
    ...
    for (size_t block_idx = 0; block_idx < block_num; block_idx++) {
        ...
        k_block_decode_cpu<RAW_BIT_LEN, HIDDEN_DIM_BLOCK_SIZE, CTX_LEN_BLOCK_SIZE, 4>(compressed_buffer, k_encode_block);
...
template void k_decode_cpu<4, 128, 128, 64>(
    const uint8_t *compressed_buffer_ptr,
    const uint32_t *block_info_buffer_ptr,
    uint8_t *quant_ints_tensor_ptr,
    size_t ctx_len
...
template void k_decode_cpu<4, 5120, 128, 64>(
    const uint8_t *compressed_buffer_ptr,
    const uint32_t *block_info_buffer_ptr,
    uint8_t *quant_ints_tensor_ptr,
    size_t ctx_len
...
void k_decode_cpu_pyi(
    ...
    k_decode_cpu_assert(quant_ints_tensor, compressed_buffer, block_info_buffer, ctx_len, hidden_dim, ctx_len_block_size, hidden_dim_block_size, bits_len);
    ...
    if (
        ...
        k_cpu::k_decode_cpu<4, 5120, 128, 64>(
        ...
        k_cpu::k_decode_cpu<4, 1024, 128, 64>(
            compressed_buffer_ptr,
            block_info_buffer_ptr,
            quant_ints_tensor_ptr,
            ctx_len
